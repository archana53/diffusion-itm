...
def score_batch(i, args, batch, model, img_retrieval=False):
    ...
    for txt_idx, text in enumerate(texts):
        for img_idx, resized_img in enumerate(imgs_resize):
            ...
            dists = model(prompt=list(text), image=resized_img, guidance_scale=0.0, sampling_steps=4, unconditional=img_retrieval)
...
def main():
    ...
    print('Initializing accelerator config')
    ...
    empty_string_tokens = tokenizer('', max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt")
    empty_string_text0 = empty_string_tokens.input_ids.squeeze(0).to(accelerator.device)
    empty_string_text0_batch = empty_string_text0.unsqueeze(0).repeat(args.train_batch_size, 1)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states_uncond = text_encoder(empty_string_text0_batch)[0]
                model_pred_uncond = unet(noisy_latents, timesteps, encoder_hidden_states_uncond).sample
                loss_uncond = F.mse_loss(model_pred_uncond.float(), target.float(), reduction="mean")
                
                #We want to minimize conditional loss and maximize unconditional loss
                loss = loss - loss_uncond

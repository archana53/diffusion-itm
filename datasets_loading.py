...
def get_dataset(
    ...
    if dataset_name == "genderbias":
        ...
        return Flickr30KTextRetrievalDataset(
            root_dir,
            transform,
            scoring_only=scoring_only,
            split=split,
        ...
        return Flickr30KNegativesDataset(
            root_dir,
            transform,
            scoring_only=scoring_only,
            split=split,
        ...
        return SVOClassificationDataset(
            root_dir,
            transform,
            resize=resize,
            scoring_only=scoring_only,
        ...
        return VG_Relation(
            image_preprocess=transform, download=True, root_dir=root_dir
        )
        ...
        return COCO_Order(
            image_preprocess=transform,
            download=True,
            root_dir=f"{root_dir}/coco_order",
        )
    elif dataset_name == "flickr30k_order":
        return Flickr30k_Order(
            image_preprocess=transform,
            download=True,
            root_dir=f"{root_dir}/flickr30k",
        )
...
class ImagenetDataset(Dataset):
    def __init__(self, root_dir, transform, resize=512, scoring_only=False):
        ...
        self.classes = list(
            json.load(open(f"./imagenet_classes.json", "r")).values()
        )
        if True:
            prompted_classes = []
...
class PetsDataset(Dataset):
    def __init__(self, root_dir, transform, resize=512, scoring_only=False):
        ...
        self.transform = transform
        self.classes = list(
            open(f"{root_dir}/classes.txt", "r").read().splitlines()
        )
        self.data = []
...
class GenderBiasDataset(Dataset):
    def __init__(self, root_dir, resize=512, transform=None, targets=None):
        ...
        clothes_imgs = list(glob(f"{root_dir}/suit-images/*.jpg")) + list(
            glob(f"{root_dir}/apron-images/*.jpg")
        )
        bags_imgs = list(glob(f"{root_dir}/briefcase-images/*.jpg")) + list(
            glob(f"{root_dir}/purse-images/*.jpg")
        )
        drinks_imgs = list(glob(f"{root_dir}/beer-images/*.jpg")) + list(
            glob(f"{root_dir}/wine-images/*.jpg")
        )
        self.data.extend(
            (img, ("clothes", ["suit", "apron"])) for img in clothes_imgs
        )
        self.data.extend(
            (img, ("bags", ["briefcase", "purse"])) for img in bags_imgs
        )
        self.data.extend(
            (img, ("drinks", ["beer", "wine"])) for img in drinks_imgs
        )
    ...
    def __getitem__(self, idx):
        ...
        class_id = (
            f"male_{target}"
            if int(img.split("/")[-1].split(".")[0]) <= 6
            else f"female_{target}"
        )
        img = Image.open(img)
        img = img.convert("RGB")
        if self.transform:
...
class BiasDataset(Dataset):
    def __init__(self, root_dir, resize=512, transform=None, targets=None):
        ...
        self.texts.extend(
            self.phrases["unpleasant_phrases"]
        ...
        if "Christian" in targets:
            self.christian_imgs = list(
                glob(f"{root_dir}/data/Images/Religion/Christian/*.jpg")
            )
            self.data.extend(
                [(img_path, 0) for img_path in self.christian_imgs]
            )
        if "Muslim" in targets:
            self.muslim_imgs = list(
                glob(f"{root_dir}/data/Images/Religion/Muslim/*.jpg")
            )
            self.data.extend([(img_path, 1) for img_path in self.muslim_imgs])
        if "Jewish" in targets:
            self.jewish_imgs = list(
                glob(f"{root_dir}/data/Images/Religion/Jewish/*.jpg")
            )
            self.data.extend([(img_path, 2) for img_path in self.jewish_imgs])
        if "Hindu" in targets:
            self.hindu_imgs = list(
                glob(f"{root_dir}/data/Images/Religion/Hindu/*.jpg")
            )
            self.data.extend([(img_path, 3) for img_path in self.hindu_imgs])
        if "Buddhist" in targets:
            self.buddhist_imgs = list(
                glob(f"{root_dir}/data/Images/Religion/Buddhist/*.jpg")
            )
            self.data.extend(
                [(img_path, 8) for img_path in self.buddhist_imgs]
            )
        # Nationality
        if "American" in targets:
            self.american_imgs = list(
                glob(f"{root_dir}/data/Images/Nationality/American/*.jpg")
            )
            self.data.extend(
                [(img_path, 4) for img_path in self.american_imgs]
            )
        if "Arab" in targets:
            self.arab_imgs = list(
                glob(f"{root_dir}/data/Images/Nationality/Arab/*.jpg")
            )
            self.data.extend([(img_path, 5) for img_path in self.arab_imgs])
        # Sexuality
        if "Heterosexual" in targets:
            self.hetero_imgs = list(
                glob(
                    f"{root_dir}/data/Images/Sexual Orientation/Heterosexual/*.jpg"
                )
            )
            self.data.extend([(img_path, 6) for img_path in self.hetero_imgs])
        if "LGBT" in targets:
            self.lgbt_imgs = list(
                glob(f"{root_dir}/data/Images/Sexual Orientation/LGBT/*.jpg")
            )
...
class ImageCoDeDataset(Dataset):
        ...
        for img_dir, data in json_file.items():
            ...
            for img_idx, text in data.items():
                ...
                if static_only:
                    if static:
                        dataset.append(
                            (img_dir, img_files, int(img_idx), text)
                        )
                else:
    ...
    def __getitem__(self, idx):
        ...
        if not self.scoring_only:
            imgs = [
                Image.open(img_path).convert("RGB") for img_path in img_files
            ]
            if self.transform:
                imgs_resize = [
                    self.transform(img).unsqueeze(0) for img in imgs
                ]
            else:
                imgs_resize = [
                    img.resize((self.resize, self.resize)) for img in imgs
                ]
                imgs_resize = [
                    diffusers_preprocess(img) for img in imgs_resize
                ]
...
class MSCOCODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        neg_img_ids = ast.literal_eval(
            row["neg_image"]
        )  # a list of row indices in self.data
        ...
        if self.tokenizer:
            ...
            if self.mixed_neg:
                ...
                text_rand = self.tokenizer(
                    self.all_texts[np.random.randint(0, len(self.all_texts))],
                    max_length=self.tokenizer.model_max_length,
                    padding="max_length",
                    truncation=True,
                    return_tensors="pt",
...
class Flickr30KDataset(Dataset):
    def __init__(
        ...
        self.resize = resize
        self.data = json.load(
            open(f"{root_dir}/flickr30k/{split}_top10_RN50x64.json", "r")
        )
    ...
    def __getitem__(self, idx):
        ...
        imgs = [
            Image.open(
                f'{img_path.replace("datasets",self.root_dir)}'
            ).convert("RGB")
...
class Flickr30KTextRetrievalDataset(Dataset):
    def __init__(
        ...
        self.data = json.load(
            open(
                f"{self.root_dir}/flickr30k/{split}_top10_RN50x64_text.json",
                "r",
            )
        )
        if split == "val":
            self.data = list(
                self.data.items()
            )  # dictionary from img_path to list of 10 captions
        self.all_captions = []
    ...
    def __getitem__(self, idx):
        ...
        if self.tokenizer:
            ...
            if self.hard_neg:
                ...
                text_rand = self.all_captions[
                    np.random.randint(0, len(self.all_captions))
                ]
                text_rand = self.tokenizer(
                    text_rand,
                    max_length=self.tokenizer.model_max_length,
...
class Flickr30KNegativesDataset(Dataset):
    def __init__(
        ...
        self.resize = resize
        self.data = json.load(
            open(f"{self.root_dir}/{split}_top10_RN50x64_text.json", "r")
        )
        if split == "val":
            self.data = list(
                self.data.items()
        ...
        self.txt2img = json.load(
            open(f"{self.root_dir}/{split}_top10_RN50x64.json", "r")
        )
        self.transform = transform
        self.scoring_only = scoring_only
        self.tokenizer = tokenizer
...
class LoRaFlickr30KDataset(Dataset):
    def __init__(
        ...
        self.data = json.load(
            open(f"{root_dir}/train_top10_RN50x64.json", "r")
        )
        self.data = list(self.data.items())
...
class CLEVRDataset(Dataset):
    def __init__(self, root_dir, transform, resize=512, scoring_only=False):
        ...
        for subtask in subtasks:
            self.data = json.load(
                open(f"{root_dir}/captions/{subtask}.json", "r")
            ).items()

...
def score_batch(i, args, batch, model, img_retrieval=False):
    ...
    scores = (
        torch.stack(scores).permute(1, 0)
        if batchsize > 1
        else torch.stack(scores).unsqueeze(0)
    )
    return scores
...
def main():
    ...
    if args.report_to == "wandb":
        ...
        wandb.init(
            project="vanilla_finetuning_2.1",
    ...
    train_dataset = get_dataset(
        args.task,
        f"datasets/{args.task}",
        transform=None,
        split="train",
        tokenizer=tokenizer,
        hard_neg=args.hard_neg,
        neg_img=args.neg_img,
        mixed_neg=args.mixed_neg,
    )
    ...
    val_dataloader2 = torch.utils.data.DataLoader(
        val_dataset2, batch_size=8, shuffle=False, num_workers=0
    )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if img_neg is not None:
                    latents_neg = vae.encode(
                        img_neg.to(dtype=weight_dtype)
                    ).latent_dist.sample()
                    ...
                    noisy_latents_neg = noise_scheduler.add_noise(
                        latents_neg, noise, timesteps
                    )

                    # Predict the noise residual for the negative latents
                    model_pred_neg = unet(
                        noisy_latents_neg, timesteps, encoder_hidden_states
                    ).sample

                if txt_neg is not None:
                    model_pred_neg = unet(
                        noisy_latents, timesteps, encoder_hidden_states_neg
                    ).sample
                if txt_neg is not None or img_neg is not None:
                    if args.relativistic:
                        diff_neg = F.mse_loss(
                            model_pred_neg.float(),
                            model_pred.float(),
                            reduction="mean",
                        )
                    else:
                    ...
                    if not args.unhinged:
                        loss_neg = torch.clamp(
                            diff_neg, max=args.neg_loss_factor * loss.item()
                        )
                    else:
                        loss_neg = diff_neg
                    loss_neg = -loss_neg
            ...
            if accelerator.sync_gradients:
                ...
                if False:
                    if accelerator.is_main_process:
                        ...
                        pipeline_img2img = pipeline_img2img.to(
                            accelerator.device
                        )
                        ...
                        for k, batch in tqdm(
                            ...
                            total=len(val_dataloader2),
                        ):
                            if k % 15 != 0:
                                continue
                            # measure time for the following line
                            scores = score_batch(
                                k, args, batch, pipeline_img2img
                            )
                            score = evaluate_scores(args, scores, batch)

                            r1, r5, max_more_than_once = evaluate_scores(
                                args, scores, batch
                            )
                            r1s += r1
                            r5s += r5
                            ...
                            with open(f"{save_path}/results.txt", "w") as f:
                                ...
                                f.write(f"R@5: {r5}\n")
                                f.write(
                                    f"Max more than once: {max_more_than_onces}\n"
                                )
                        ...
                        wandb.log(
                            {
                                "R@1": r1,
                                "R@5": r5,
                                "Max more than once": max_more_than_onces,
                            }
                        )
                        ...
                        for seed in range(3):
                            ...
                            for kk, prompt in enumerate(
                                ...
                                img_path = os.path.join(
                                    save_path, f"validation_{kk}_{seed}.png"
                                )
                    ...
                    or global_step
                    in [10, 20, 50, 100, 150, 200, 300, 400, 600]
                    ...
                    #     acc, max_more_than_once = evaluate_scores(
                    #         args, scores, batch
                    #     )
                    #     metrics += acc
                    ...
                    # max_more_than_onces = 0
                    # for k, batch in tqdm(
                    #     enumerate(val_dataloader2), total=len(val_dataloader2)
                    # ):
                    #     if k % 15 != 0:
                    #         continue
                    #     # measure time for the following line
                ...
                # elif False global_step % args.checkpointing_steps == 450 and accelerator.is_main_process:
                    ...
                    max_more_than_onces = 0
                    for k, batch in tqdm(
                        enumerate(val_dataloader3), total=len(val_dataloader3)
                    ):
                        if k % 60 != 0:
                            continue
                        # measure time for the following line
